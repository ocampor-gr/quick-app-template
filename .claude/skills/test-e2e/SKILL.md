...
```bash
rm -rf /tmp/out/quick-app
cruft create . --output-dir /tmp/out --no-input
```

### 2. Create the `.env` file

```bash
cat > /tmp/out/quick-app/.env << 'EOF'
...
```bash
cd /tmp/out/quick-app && docker compose build && docker compose up -d
```
...
All three containers must be running: nginx proxy (port 80), frontend (port 3000), backend (port 8000).
...
```bash
cd /tmp/out/quick-app && docker compose down
rm -rf /tmp/out/quick-app
rm -f test-*.png
rm -rf .playwright-mcp/
```

If any test **failed**, keep the screenshots, logs, and rendered project so the user can inspect them.
